...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleIgnoreRequirements,                        //262
    ...
    else
    {
        ...
        switch(GetId())
        {
            ...
            case 55359:                                     // Living Bomb Rank2
                caster->CastSpell(m_target,55361,true,NULL,this); return;
            case 55360:                                     // Living Bomb Rank3
                caster->CastSpell(m_target,55362,true,NULL,this); return;
            case 27243:                                     // Seed of Corruption Rank1
                caster->CastSpell(m_target,27285,true,NULL,this); return;
            case 47835:                                     // Seed of Corruption Rank2
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            if(m_modifier.m_auraname == SPELL_AURA_PERIODIC_DAMAGE)
            {
                ...
                else if (GetSpellProto()->SpellFamilyName==SPELLFAMILY_WARLOCK && GetSpellProto()->SpellFamilyFlags & 0x0000000000004000LL && (m_target->GetHealth() <= m_target->GetMaxHealth() * 0.25f))
                    pdamage *= 4;
            }
            else
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            // ignore item heals
            if (!m_castItemGuid)
                pCaster->ProcDamageAndSpell(m_target, procAttacker, procVictim, PROC_EX_NORMAL_HIT, gain, BASE_ATTACK, GetSpellProto());
            break;
        }
        case SPELL_AURA_PERIODIC_MANA_LEECH:
        {
            ...
            if(GetSpellProto()->Id == 5138 || GetSpellProto()->Id == 3034)
            {
               int32 drain = m_target->GetMaxPower(power) * 3 /100;
               uint32 maxp = GetCaster()->GetMaxPower(power) * 3 / 50;

               drain_amount = drain > maxp ? maxp : drain;
            }
...
void Aura::HandleAuraConvertRune(bool apply, bool Real)
{
    ...
    for(uint32 i = 0; i < MAX_RUNES; ++i)
    {
        ...
        {
            if(plr->GetCurrentRune(i) == RuneType(GetSpellProto()->EffectMiscValueB[m_effIndex]) && plr->GetBaseRune(i) != RuneType(GetSpellProto()->EffectMiscValueB[m_effIndex]))
            {
...
void Aura::HandleIgnoreRequirements(bool apply, bool real)
{
    if (!real)
        return;
