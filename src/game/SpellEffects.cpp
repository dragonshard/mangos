        ...
        switch(m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARLOCK:
            {
                ...
                else if (m_spellInfo->TargetAuraState == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    if (aura)
                    {
                        int32 damagetick = m_caster->SpellDamageBonus(unitTarget, aura->GetSpellProto(), aura->GetModifier()->m_amount, DOT, 1, false);
                        damage += damagetick * 4;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            else if(m_spellInfo->SpellFamilyFlags & UI64LIT(0x0001000000000000))
            {
                ...
                std::list<Unit*> targets;
                {
                    ...
                    cell_lock->Visit(cell_lock, grid_object_checker,  *target->GetMap());
                    cell_lock->Visit(cell_lock, world_object_checker, *target->GetMap());
                }
...
void Spell::EffectPowerBurn(uint32 i)
{
    ...
    if(m_spellInfo->Id == 8129)
    {
       ...
       uint32 maxp = GetCaster()->GetMaxPower(powertype) * m_spellInfo->EffectBasePoints[i] / 50;

       power = power > maxp ? maxp : power;
    }
...
void Spell::EffectHeal( uint32 /*i*/ )
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND) && !m_caster->HasAura(54824))
        {
